...
describe('Efetuar Login', () => {
  ...
  it('CT014 - Validar efetuar login com sucesso', () => {
    cy.fixture('usuario.data.json').then(data => {
      ...
      cy.get('.sc-bcPKhP').should('contain', 'Olá, ' + data.usuario[0].nomeCompleto)
    })
  })
  ...
  it('CT015.4 - Validar efetuar login com e-mail em formato inválido', () => {
    cy.fixture('usuario.data.json').then(data => {
      cy.efetuarLogin(data.usuario[0].nomeCompleto, data.usuario[0].senha)
  ...
  it('CT015.6 - Validar efetuar login com senha incorreta', () => {
    cy.fixture('usuario.data.json').then(data => {
      cy.efetuarLogin(data.usuario[0].email, data.usuario[1].senha)
      cy.get('.Toastify__toast-body > :nth-child(2)').should('contain', 'Usuário e/ou senha incorretos')
    })
  })
